...
const standardizePhoneNumbers = (arr) => {
    ...
    return arr.map((value, idx, array)=>{
        return value.substring(1, 4).concat(value.substring(6, 9)).concat(value.substring(10, 14));
    });
};
...
const onlyOddChars = (str) => {
    // Solution code here...
    return str.split('').filter((val, idx, array)=>{
        if (idx % 2 !== 0){
            return val;
        }
    }).join('');
...
const allHappy = (arr) => {
    // Solution code here...
    return arr.every((val, idx, array)=>{
        return val.includes(':)');
    });
};
...
const unenrollBrook = (arr) => {
    // Solution code here...
    return arr.map((val, idx, array)=>{
        return val.filter((val2, idx, array)=>{
            return !val2.includes('Brook');
        });
    });
};
