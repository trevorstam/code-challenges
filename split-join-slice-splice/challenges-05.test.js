...
const wordsToCharList = (arr) => {
    ...
    for (let i = 0; i < arr.length; i ++){
        letters = arr.charAt(i);
        newList.push(letters);
    }
    return newList;
};

// alternatively and much quicker => arr.split(' ');
...
const listFoods = (recipe) => {
    let result = [];
    // Solution code here...
   recipe.ingredients.forEach((value, idx, array) =>{
       let arrayBreaker = value.slice(value.indexOf(' ') + 1);
       let arrayBreaker2 = arrayBreaker.slice(arrayBreaker.indexOf(' ') + 1);
       result.push(arrayBreaker2);
   })
    return result;
}
...
const splitFoods = (recipe) => {
    ...
    recipe.ingredients.forEach((value, idx, array)=>{
        ...
        let ingred = splitString.slice(2);
        let ingredJoined = ingred.join(' ');
        result.push(ingredJoined);
    })
    return result;
}
...
const stepActions = (recipe) => {
    let result = [];
    // Solution code here...
    recipe.steps.forEach((value, idx, array)=>{
        let actionWords = value.split(' ')[0];
