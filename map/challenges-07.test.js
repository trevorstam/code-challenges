...
const charCode = (arr) => {
    // Solution code here...
    return arr.map((acc, cur) => {
        return acc.charCodeAt(arr);
    })
};
...
const evenOdd = (arr) => {
    // Solution code here...
    return arr.map((acc, cur) => {
        if (acc % 2 === 0 && Number.isInteger(acc)) {
            return 'even';
        } else if (acc % 2 !== 0 && Number.isInteger(acc)) {
            return 'odd';
        } else {
            return 'N/A';
        }
