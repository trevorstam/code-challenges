...
const count = (target, input) => {
  ...
  return input.reduce((acc, cur)=>{
      return acc.concat(cur);//create new array by concatenating 2 into 1
  }, []).reduce((acc, cur)=>{
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc, cur)=>{
      return acc.concat(cur);//create 1 array
  }, []).reduce((acc, cur)=>{
      return acc + cur;//add all numbers in array
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map((value, idx, array)=>{//apply method on each element in base array
      return value.filter((num, idx, array) => { //filter out values in nested arrays
          return Number.isInteger(num) === true && num % 5 === 0;
      }).map((powerNum, idx, array)=>{ //perform math operations on remaining values in nested arrays
          return Math.pow(2, powerNum);
      })
  })
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter((value, idx, array)=>{
      return value.gender === 'male'|| value.gender == 'female';
  }).map((swChar, idx, array)=>{
      return swChar.name;
  }).join(' and ');
};
